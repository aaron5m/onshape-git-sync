...
def generate_assembly_images(document_id, version, element_id):
    """
    ...
    """
    # Get version id and timestamp folder
    version_id = version.get("id")
    timestamp_folder_name = get_version_timestamp_folder_name(version)
    
    if not version_id or not timestamp_folder_name:
        print(f"Invalid metadata (id or timestamp) for version {version.get('name')}")
        return
...
def capture_all_assemblies_images_for_version(document_id, version):
    """
    ...
    elements_file_path = f"{SNAPSHOT_DIR}/{timestamp_folder_name}/elements.json"
    ...
    try:
        ...
        for assembly in assemblies:
            print(f"Generating images for assembly: {assembly['name']}")
            generate_assembly_images(document_id, version, assembly['id'])
